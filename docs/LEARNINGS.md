...
## L-021: Verify a channel's ids against `client_crosswalk` before carving a view — a client can have different ids per channel and dirty ids in the Sheet
...
## L-024: A second unauthenticated data path was found and fixed the same week as the Cloudflare Access hole — assume there may be more

**Context (session 2026-07-13):** `budget-planning.html` originally fetched its data from an n8n webhook with no authentication of its own — it relied entirely on the page being behind Cloudflare Access to be "protected." That's a real protection for a human clicking through a browser, but a webhook URL itself, if ever called directly (leaked, guessed, or hit from a context that bypasses Access), had zero independent access control. It was fixed by moving budget-planning's data fetch to `functions/api/budget-events.js`, a Pages Function that authenticates directly to BigQuery with its own service-account JWT (ADR-013) and additionally checks the caller's identity/capabilities — so the data path is now protected at two independent layers, not one.

**Why this matters beyond the one fix:** this is the **second** "assumed protected, actually wasn't" finding in the same week, after the Cloudflare Access apex-hostname gap. Neither was caused by carelessness in the current session — both were pre-existing gaps inherited from earlier, faster iterations of the platform. The pattern is: a webhook or URL that "only the app calls" is not the same as "only authorized humans can trigger it," and the difference matters even when the frontend is gated.

**Rule:** when auditing Cortex (or any similar system) for security, don't stop at "is the page behind Access" — separately ask, for every data-fetching or data-writing endpoint the page calls: **would this URL do something (or reveal something) if called directly, with no browser, no session, no Access login?** Any n8n webhook, any Pages Function, any API route should be independently authorized (checking the Access-injected identity header, or its own credential), not merely "reached only via a page that happens to be gated." Treat this as a standing checklist item — there may be other webhooks in the n8n instance (tickets, roadmap-agent, etc.) that haven't been audited this way yet (P-TECH-19-era note).

---

## L-025: Committed and actual use different client-identity mechanisms — this is a root cause of desync, not a quirk

**Context (session 2026-07-30, Norfolk):** the pacing pipeline resolves the client name two different ways depending on the metric.
- **Actual spend** joins by `customer_id` against `client_crosswalk` (`spend_daily_unified` does `COALESCE(x.canonical_client, "UNMAPPED...")` on `customer_id`). Robust: to rename or re-split a client you change one crosswalk row and every view reflects it instantly.
- **Committed budget** carries the client *name* as free text — written directly in the Sheet (`committed_budget_long`) or in a `budget_events` row. Fragile: the name is hardcoded where it was entered, joined by string equality (`k.client = e.client`) in `budget_base_current`.

**Consequence:** when a client is renamed or split (Norfolk → Virginia/Nebraska), fixing the crosswalk instantly fixes the *actual* side but does nothing for the *committed* side, because committed doesn't read the crosswalk — it matches on a name that now doesn't exist. You end up with "ghost" rows (committed with $0 actual under the old name) and real rows (actual with $0 committed under the new name) until the committed name is also changed, in whatever source(s) hold it.

**Rule:** whenever a client is renamed or split, remember there are **two identity systems** to reconcile, not one. The crosswalk fixes actual; the committed name must be changed separately in every source that holds it (the Sheet AND any `budget_events` rows). The real fix of record is to resolve committed by `customer_id` against the crosswalk too, giving a single source of client identity — tracked as P-TECH-20. Until then, expect this class of bug to recur with any client that shares a name with another (Norfolk was the first, not the last).

## L-026: Measure before reacting — an un-measured "this is huge, fix it now" is the dangerous move

**Context (session 2026-07-30):** mid-Norfolk, the discovery that one client's committed came from the old Sheet triggered the conclusion "the Sheet is obsolete, AMs edit everything in Cortex now, we need to fix this whole thing immediately." That premise was **false**, and acting on it (e.g. dropping the Sheet from the `COALESCE` in `budget_base_current`, or forcing a mass migration) would have broken the committed budget of 67 clients. A single `COUNT(*) ... GROUP BY source` query settled it in seconds: 20 clients on the editor, **67 still on the Sheet**. The Sheet is not obsolete — it's the live fallback for 77% of clients.

**Rule:** when something *feels* like a large, urgent, structural problem, the instinct to "fix it now" is exactly when to stop and measure first. Reach for the cheapest query that sizes the problem (a COUNT, a DISTINCT, a GROUP BY) before designing any fix. The correct response to a 2-client problem and a 67-client problem is completely different, and you cannot tell which you have from the feeling — only from the number. A withheld hour of "fixing" costs nothing; a structural change made on an un-measured premise can break the majority of the system. This applies doubly at the end of a long session when the temptation is to close everything.

## L-027: `budget_base_current` is a Sheet→Editor migration bridge — the `source` column tells you which path each row took
