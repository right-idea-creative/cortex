...
- **Correction (2026-06-17, session 8):** an earlier note here claimed the production HTML was uploaded by Nate outside the repo's auto-deploy and that the repo copy was stale. That was verified false on this date: Cloudflare Pages deploys from `right-idea-creative/cortex` on push to `main` (automatic deployments; no Direct Uploads in the deployment history), so **the repo is the source of truth** and the frontend lives in version control. The frontend therefore is *not* the "source not in git" hazard it was described as. (P-TECH-09 closed accordingly.)
